...
import static dev.openrealm.launcher.bridge.ZLBridgeStatesKt.CURSOR_DISABLED;
import static dev.openrealm.launcher.bridge.ZLBridgeStatesKt.CURSOR_ENABLED;
...
public class CallbackBridge {
    ...
    public static boolean notifyLauncher(int type, int... action) {
        if (action == null || action.length == 0) {
            LoggerBridge.append("OpenRealmLauncher: SDL notification has no action");
            return false;
        }
        switch (type) {
            case NOTIF_TYPE_SDL:
                if (action[0] == ACTION_INIT_LAUNCHER_INTEGRATION) {
                    ...
                    try {
                        LoggerBridge.append("OpenRealmLauncher: loading SDL3");
                        System.loadLibrary("SDL3");
                        LoggerBridge.append("OpenRealmLauncher: loading SDL2");
                        System.loadLibrary("SDL2");
                        LoggerBridge.append("OpenRealmLauncher: setting up SDL JNI");
                        SdlBridge.setupJNI();
                        LoggerBridge.append("OpenRealmLauncher: binding SDL surface");
                        ...
                        LoggerBridge.append("OpenRealmLauncher: SDL support enabled!");
                        return true;
                    } catch (Throwable e) {
                        ...
                        LoggerBridge.append("OpenRealmLauncher: SDL launcher integration is unavailable:\n" + trace);
                    }
                }
    ...
    private static void onDirectInputEnable() {
        LoggerBridge.append("OpenRealmLauncher: Direct gamepad input enabled");
