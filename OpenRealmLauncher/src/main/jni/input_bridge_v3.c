...
JNIEXPORT void JNICALL
Java_dev_openrealm_launcher_game_sdl_SdlBridge_initializeControllerSubsystems(__attribute__((unused)) JNIEnv* env, __attribute__((unused)) jclass clazz) {
...
JNIEXPORT jboolean JNICALL
Java_dev_openrealm_launcher_game_sdl_SdlBridge_isSdlRenderActive(__attribute__((unused)) JNIEnv* env, __attribute__((unused)) jclass clazz) {
    // SDL 渲染路径以首个 SDL 窗口创建为标志；仅手柄子系统初始化 SDL（MC 26.2 挂 Controlify）时无窗口
    return sSdlPrimaryWindow != NULL ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL
Java_dev_openrealm_launcher_game_sdl_SdlBridge_setNativeTextInputActive(__attribute__((unused)) JNIEnv* env, __attribute__((unused)) jclass clazz, jboolean active) {
...
static bool tryCriticalNative(JNIEnv *env) {
    ...
    jclass criticalNativeTest = (*env)->FindClass(env, "dev/openrealm/launcher/game/input/CriticalNativeTest");
